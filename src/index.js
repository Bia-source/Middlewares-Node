...
function checksCreateTodosUserAvailability(request, response, next) {
   ...
   if(!user.pro && user.todos.length >= 10){
    return response.status(403).json({ error: "voce possui uma conta gratis e 10 todos cadastrados"});
   }
    return next()
  }
...
function checksTodoExists(request, response, next) {
   ...
   if(!verifyUser){
    return response.status(404).json({ error: "user nao existe"});
   }
   ...
   //verificando todo utilizando o usuario encontrado
   const verifyTodo = verifyUser.todos.find((todo) => todo.id === id);
   if(!verifyTodo){
